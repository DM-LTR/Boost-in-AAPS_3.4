...
@Singleton
open class OpenAPSBoostV3MLG3Plugin @Inject constructor(
    ...
    // Health Connect into AAPS's local HR table. Pulled each Boost cycle (throttled
    // internally to ApsBoostHealthConnectPollMin).
    private val healthConnectHrIngest: app.aaps.plugins.aps.openAPSBoost.HealthConnectHrIngest,
) : PluginBase(
    PluginDescription()
        .mainType(PluginType.APS)
        .fragmentClass(OpenAPSFragment::class.java.name)
        .pluginIcon(app.aaps.core.ui.R.drawable.ic_generic_icon)
        .pluginName(R.string.openaps_boost_v3mlg3)
        .shortName(R.string.boost_v3mlg3_shortname)
        .preferencesId(PluginDescription.PREFERENCE_SCREEN)
        .preferencesVisibleInSimpleMode(false)
        .showInList { false } // retired: superseded by Boost V6 — hidden, no longer a selectable option
        .description(R.string.description_boost_v3mlg3),
    aapsLogger, rh
    ...
    private fun calculateBoostIsf(
        ...
        if (useTdd) {
            ...
            if (tdd7D != null && tdd1D != null && tddLast24H != null && tddLast4H != null && tddLast8to4H != null && tdd7D > 0) {
                ...
                if (tddWeightedFromLast8H < (0.75 * tdd7D)) {
                    ...
                    tdd = tdd7D
                    debug.append("\n7D-only TDD (no blend)")
                }
                debug.append("\nTDD=${Round.roundTo(tdd, 0.1)}")
                ...
                if (tdd > 0 && logTerm > 0) {
                    ...
                    aapsLogger.warn(LTag.APS, "BoostV3ML TDD ISF: invalid tdd=$tdd or logTerm=$logTerm, falling back to profile ISF")
                }
            } else {
                debug.append("\n⚠ TDD data incomplete — using profile ISF")
                aapsLogger.debug(LTag.APS, "BoostV3: TDD parts missing (7D=$tdd7D 1D=$tdd1D 24H=$tddLast24H 4H=$tddLast4H 8-4H=$tddLast8to4H)")
            }
        } else {
        ...
        if (isTempTarget && ((highTtRaisesSens && targetBg > bgNormalTarget) || (lowTtLowersSens && targetBg < bgNormalTarget))) {
            ...
            if (c * (c + targetBg - bgNormalTarget) > 0.0) {
                ...
                aapsLogger.debug(LTag.APS, "BoostV3ML ISF adjusted by ${1.0 / ratio} due to TT of ${targetBg.toInt()}")
            }
        }
        ...
        aapsLogger.debug(LTag.APS, "BoostV3ML ISF: $debug")
    ...
    override fun invoke(initiator: String, tempBasalFallback: Boolean) {
        ...
        // Runs asynchronously so the cycle is never blocked by an HC read.
                ...
                if (sleepResult.transitioned) {
                    ...
                    val historyChanged = when {
                        prevSleepState != app.aaps.plugins.aps.openAPSBoost.SleepStateDetector.SleepState.SLEEPING &&
                            ...
                            sleepHistoryCached = app.aaps.plugins.aps.openAPSBoost.SleepHistoryTracker.onSleepStart(sleepHistoryCached, now)
                            true
                        }
                        prevSleepState == app.aaps.plugins.aps.openAPSBoost.SleepStateDetector.SleepState.SLEEPING &&
                            newState != app.aaps.plugins.aps.openAPSBoost.SleepStateDetector.SleepState.SLEEPING -> {
    ...
    override fun addPreferenceScreen(preferenceManager: PreferenceManager, parent: PreferenceScreen, context: Context, requiredKey: String?) {
        ...
        category.apply {
            key = "openapsboostv3ml_settings"
            title = rh.gs(R.string.openaps_boost_v3ml)
            ...
            // ── 3. Dynamic ISF Controls (V3: no velocity setting) ────────
            addPreference(preferenceManager.createPreferenceScreen(context).apply {
